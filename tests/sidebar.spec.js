...
// Right-click somewhere inside #sidebar that's NOT a tj_description node, so
...
async function rightClickNode(page, name) {
    await page.locator(`#tree .tj_description:text-is("${name}")`).click({button: 'right'});
    await expect(page.locator('.sidebar-ctx-menu')).toBeVisible();
}
...
test('root ctx menu: new file creates a root-level file', async ({page}) => {
    ...
    await expect(page.locator('#tree .tj_description:text-is("MyRootFile")')).toBeVisible();
});
...
test('root ctx menu: new dir creates a root-level directory', async ({page}) => {
    ...
    await expect(page.locator('#tree .tj_description:text-is("MyRootDir")')).toBeVisible();
});
...
test('file ctx menu: new file creates sibling in parent dir', async ({page}) => {
    ...
    await expect(page.locator('#tree .tj_description:text-is("SiblingFile")')).toBeVisible();
});
...
test('file ctx menu: new dir creates sibling dir in parent dir', async ({page}) => {
    ...
    await expect(page.locator('#tree .tj_description:text-is("SiblingDir")')).toBeVisible();
});
...
test('file ctx menu: rename', async ({page}) => {
    ...
    await expect(page.locator('#tree .tj_description:text-is("Renamed")')).toBeVisible();
    await expect(page.locator('#tree .tj_description:text-is("README")')).toHaveCount(0);
});
...
test('dir ctx menu: new file creates file inside directory', async ({page}) => {
    ...
    await page.locator('#tree .tj_description:text-is("life")').click();
    await expect(page.locator('#tree .tj_description:text-is("InsideLife")')).toBeVisible();
});
...
test('dir ctx menu: new dir creates sub-directory', async ({page}) => {
    ...
    await page.locator('#tree .tj_description:text-is("life")').click();
    await expect(page.locator('#tree .tj_description:text-is("SubDir")')).toBeVisible();
});
...
test('dir ctx menu: rename', async ({page}) => {
    ...
    await expect(page.locator('#tree .tj_description:text-is("happiness")')).toBeVisible();
    await expect(page.locator('#tree .tj_description:text-is("life")')).toHaveCount(0);
});
